...
class SheetExportconfig extends FormApplication {
	...
	async createForm(buffer) {
		...
		// This build only supports PF2E RU mapping.
		...
		for (let pfdFileIndex = 0; pfdFileIndex < mappingClass.pdfFiles.length; pfdFileIndex++) {
			...
			fields.forEach(field => {
				...
				try {
					// Some template widgets have inverted rect coordinates (negative width/height),
					// which causes "text visible only after click" in certain PDF viewers.
